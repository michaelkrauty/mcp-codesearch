...
- **The test suite no longer depends on the developer's ambient environment to pass.** Settings read the embedding dimension from the environment once, at import, and left it at zero when unset, so every test that stored a vector failed with `embedding_dim not yet initialized` — 61 failing or erroring tests on a clean checkout. Tests that build their own vectors only need the dimension to be some consistent number, so the suite defaults one. An explicitly exported dimension still wins, because a developer running against a real embedding service needs the dimension that service actually returns.
- **Tests that require a live embedding service are skipped when one is not reachable, rather than failing.** They exercise indexing and search for real, so without the service they failed at the first embedding call, which says nothing about the code under test. `requires_full_stack` already existed for exactly this purpose but was defined privately inside one test module; it now lives in the integration `conftest` alongside `requires_qdrant` and is applied wherever it belongs. A clean checkout with no services reports 677 passed and 61 skipped; with Qdrant and an embedding service reachable, all 738 run and pass.
...
- **A tool failure whose exception wraps another now names the wrapped cause instead of only the outer type.** `tool_error_handler` substitutes a message whenever an exception's own `__str__` is empty, but it named just that exception's type, which is not always enough to identify what went wrong. `qdrant_client` reports a transport failure as `ResponseHandlingException(ReadTimeout(''))`, where the wrapper and the exception it carries are both empty-messaged, so the client saw `ResponseHandlingException raised with no message` — a message that cannot distinguish a timeout from a refused connection or a malformed response, and that leaves reading the server's stderr as the only way to find out which. The chain is now walked and named, so the same failure reports `ResponseHandlingException <- ReadTimeout raised with no message`.
  ...
  The chain is followed through an explicit `raise ... from`, then through a wrapper holding its cause as its sole argument, and only then through the implicit handling context, which is the order of how deliberately each states a cause. A context suppressed by `raise ... from None` is not reported, since that syntax exists to say the context is not the cause. Traversal stops after four links and on any cycle, so an unusual chain cannot produce an unbounded message or fail to terminate.

  Describing a failure cannot itself fail: an exception whose own `__str__` raises is reported by type alone rather than allowing that second exception to escape and be reported in place of the fault actually being handled.
...
- **Definitions that share a source line are no longer dropped from the index.** A chunk's point ID was derived from its type, path and start line alone. The chunker emits a chunk for a container and another for each definition inside it, so a one-line declaration such as `struct W { void render() {} };`, `class A { foo() {} bar() {} }`, or two arrow functions on one line produced several chunks with identical IDs. Qdrant is keyed on point ID, so only the last one written survived and the rest were silently discarded. Chunk IDs now also carry the chunk's position within its file, which is unique by construction. The loss was invisible in the tool output because chunks are counted before the upsert, so the indexer reported storing every chunk while storing fewer. This is not limited to minified assets: idiomatic single-line C++ structs and compact Java and JavaScript classes all hit it. Python is largely immune because its syntax forces definitions onto separate lines.
  ...
  Existing collections need no migration to stay consistent: every indexing path replaces a file's points by deleting them for that path and then upserting, so a file picks up the new IDs the next time it changes, no old-scheme point outlives its file, and file-level point IDs are unchanged. Chunks already lost to a collision come back when their file is re-indexed.

  The shared sparse vocabulary is a different matter. A previously collided file registered a vocabulary document for every chunk the chunker emitted, including the ones Qdrant then discarded, but the removal side can only recover the chunks that survived. Re-indexing such a file therefore adds more documents than it retires, inflating this codebase's document count and the document frequency of the lost chunks' terms, which skews ranking for every codebase sharing the vocabulary database. Running `force_reindex` once on any codebase that contains same-line definitions rebuilds its contribution from scratch and clears that drift; incremental re-indexing alone does not.

### Changed

- `QdrantStorage.upsert_chunk` takes a required `ordinal` argument, the chunk's position within its file, for the same reason. It and `upsert_batch` are write primitives that do not delete anything first; replacing a file's points is the caller's job, as every indexing path in this package already does.
